...
for a0 in range(t):
    ...
    #value of the tuple (the price). I then take that sorted array of tuple2 objects, and make it look nicer.
    final = list(map(lambda x: x.tupl,sorted([tuples2(i,j) for i,j in enumerate(a)],key=tuples2.comparitor)))
        ...
        if(final != []):
            if(m<final[-1][1]):
                ...
                if(len(list(filter(lambda x:x[1]==m-final[-1][1],final)))==0 and bought == 0):
                    ...
                    m-=final[-1][1]
                    bought+=1
                    ice_cream.append(str(final[-1][0]+1)) #plus 1 due to indexing
